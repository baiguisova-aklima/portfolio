...
import { DIALOG, GAME_TEXT, GUEST_TEXT, OBJECTIVES } from './gameData';
...
type Mode = 'start' | 'intro' | 'playing' | 'dialog' | 'transition' | 'ending' | 'fetch' | 'spill' | 'restore' | 'pot' | 'balcony' | 'note' | 'eating' | 'final';
...
function targetFor(scene:number,step:number):Target|null{
  ...
  if(scene===5)return step===0?{x:510,y:405,key:'leiaJoin',label:'ПОЗНАКОМИТЬСЯ С ЛЕЕЙ'}:step===1?{x:710,y:405,key:'drakeJoin',label:'ПОЗНАКОМИТЬСЯ С ДРЕЙКОМ'}:{x:225,y:380,key:'pool',label:'СОБРАТЬ PARTY'};
...
export default function CouponExperience(){
  ...
  const goScene=useCallback((next:number)=>{player.current={...STARTS[next]};setScene(next);setStep(0);setMode('transition');beep(180,.12);},[beep]);
  const interact=useCallback(()=>{if(actionLock.current)return;const s=stateRef.current;
    if(s.mode==='transition'){setMode('playing');return;}
    ...
    if(s.mode==='dialog'){
      ...
      if(s.scene===3){actionLock.current=true;keys.current.clear();later(()=>{actionLock.current=false;},1100);}
    ...
    else if(s.scene===4){if(s.step===0)showDialog(DIALOG.bike,()=>setStep(1));else showDialog(DIALOG.shrimp,()=>goScene(5));}
    else if(s.scene===5){if(s.step===0)showDialog(DIALOG.leiaJoin,()=>setStep(1));else if(s.step===1)showDialog(DIALOG.drakeJoin,()=>setStep(2));else showDialog(DIALOG.barcelona,()=>goScene(6));}
    else if(s.scene===6)showDialog([...DIALOG.rejections,...DIALOG.support],()=>goScene(7));else if(s.scene===7)showDialog(DIALOG.wedding,()=>goScene(8));else if(target.key==='ball'){keys.current.clear();setMode('fetch');later(()=>showDialog(DIALOG.fetch),3900);}else if(target.key==='sofa'){keys.current.clear();setMode('ending');later(()=>setMode('final'),4200);beep(740,.2);}else {const lines=DIALOG[target.key as keyof typeof DIALOG];if(Array.isArray(lines))showDialog(lines);}
  ...
  useEffect(()=>{
    ...
    const loop=(now:number)=>{
      ...
      renderGame(ctx,{...state,cleaner:state.mode==='spill'||state.mode==='restore'?cleanerOrigin.current:cleanerAt(now-cleanerEpoch.current),line:animation.current.line,dialogIndex:animation.current.dialogIndex,lineElapsed:now-animation.current.lineStarted,elapsed:now-animation.current.started,action:now-animation.current.action,player:{...player.current,moving:!!(dx||dy)&&!reduced.matches,facing:animation.current.facing}},reduced.matches?0:now);
  ...
  return <main className={styles.page}><div className={styles.scanlines} aria-hidden="true"/><section className={styles.gameShell} aria-label="ALEKSEI: LEVEL 38 — пиксельная игра-подарок">
    ...
    <div className={`${styles.screen} ${scene===3?styles.guestScreen:''}`}><canvas ref={canvasRef} width={W} height={H} aria-label={`Сцена ${scene+1}: ${GAME_TEXT.scenes[scene].title}`}/>
